...
class LoRaService:
    """LoRa messaging skeleton with mock transport and ACK/retry handling.
    ...
    def handle_ack(self, raw_message: str) -> bool:
        ...
        Expected ACK payload shape (skeleton):
    ...
    def calculate_crc(self, payload: str) -> str:
        """Simple XOR checksum for message integrity in skeleton mode."""
        checksum = 0
    ...
    def _transmit(self, queued: QueuedCommand) -> None:
        ...
        if self.mock_mode:
            # In mock mode we loop message frames back locally and auto-ack.
            self._incoming_queue.put(queued.payload)
            ack_payload = (
                f"AOJ|{queued.device_id}|ACK|OK|{queued.message_id}"
            )
            ack_crc = self.calculate_crc(ack_payload)
            self._incoming_queue.put(f"{ack_payload}|{ack_crc}")

        # TODO: Replace mock behavior with SX1262 send() call.
...
_service = LoRaService()
_service.start()


def send_command(device_id: str, command: str, value: str = "") -> str:
    return _service.send_command(device_id, command, value)


def receive_message() -> str | None:
    return _service.receive_message()


def handle_ack(raw_message: str) -> bool:
    return _service.handle_ack(raw_message)


def request_device_status(device_id: str) -> str:
    return _service.request_device_status(device_id)


def broadcast_message(command: str, value: str = "") -> list[str]:
    return _service.broadcast_message(command, value)


def calculate_crc(payload: str) -> str:
    return _service.calculate_crc(payload)
